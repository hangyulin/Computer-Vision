...
def imageBoundingBox(img, M):
    ...
    #TODO-BLOCK-BEGIN
    # 4 corners coor
    corner_coor = np.array([[0, 0, 1], [0, img.shape[0] - 1, 1],
     [img.shape[1] - 1, 0, 1], [img.shape[1] - 1, img.shape[0] - 1, 1]]).T
    # See where the coor goes after transforming
    corner_coor_tranformed = M.dot(corner_coor)
    # Divide by z
    corner_coor_tranformed = corner_coor_tranformed / corner_coor_tranformed[2]

    minX = np.min(corner_coor_tranformed[0])
    minY = np.min(corner_coor_tranformed[1])
    maxX = np.max(corner_coor_tranformed[0])
    maxY = np.max(corner_coor_tranformed[1])
...
def accumulateBlend(img, acc, M, blendWidth):
    ...
    for w in range(minX, maxX):
        for h in range(minY, maxY):
            ...
            if x >= img.shape[1] - blendWidth and x <= img.shape[1]:
                z = min((img.shape[1] - x) / blendWidth, z)
            
            
            acc[h, w, 3] += z

            acc[h, w, :3] += img[y, x, :3] * z
...
def normalizeBlend(acc):
    ...
    # fill in this routine..
    #TODO-BLOCK-BEGIN
    return_img = np.zeros((acc.shape[0], acc.shape[1], 3), dtype='uint8')
    for w in range(acc.shape[1]):
        for h in range(acc.shape[0]):
            z = acc[h, w, 3]
            if z > 0:
                return_img[h, w] = acc[h, w, :3] // z
    #TODO-BLOCK-END
    # END TODO
    return return_img
...
def getAccSize(ipv):
    ...
    for i in ipv:
        ...
        #TODO-BLOCK-BEGIN
        minXt, minYt, maxXt, maxYt = imageBoundingBox(img, M)
        minX = min(minX, minXt)
        minY = min(minY, minYt)
        maxX = max(maxX, maxXt)
        maxY = max(maxY, maxYt)
