...
def alignPair(f1, f2, matches, m, nRANSAC, RANSACthresh):
    ...
    max_count = 0
    inlier_list = []
    for i in range(0, nRANSAC):
        M = leastSquaresFit(f1, f2, matches, m, random.sample(range(len(matches)), 4))
        cur_inlier_list = getInliers(f1, f2, matches, M, RANSACthresh)
        if len(cur_inlier_list) > max_count:
            max_count = len(cur_inlier_list)
            inlier_list = cur_inlier_list
    M = leastSquaresFit(f1, f2, matches, m, inlier_list)
    #TODO-BLOCK-END
    #END TODO
    return M
...
def getInliers(f1, f2, matches, M, RANSACthresh):
    ...
    for i in range(len(matches)):
        ...
        # raise Exception("TODO in alignment.py not implemented")
        m = matches[i]
        (x, y) = f1[m.queryIdx].pt
        f1_pt = np.array([x, y, 1]).T
        trans_f1 = np.dot(M, f1_pt)
        trans_f1 /= trans_f1[-1]
        f1_pt = trans_f1
        (x_2, y_2) = f2[m.trainIdx].pt
        f2_pt = np.array([x_2, y_2, 1]).T
        if np.linalg.norm(f2_pt - f1_pt) <= RANSACthresh:
...
def leastSquaresFit(f1, f2, matches, m, inlier_indices):
    ...
    if m == eTranslate:
        ...
        #TODO-BLOCK-BEGIN
        # raise Exception("TODO in alignment.py not implemented")
        cur_matches = np.take(matches, inlier_indices)
        M = computeHomography(f1, f2, cur_matches)
        #TODO-BLOCK-END
        #END TODO
